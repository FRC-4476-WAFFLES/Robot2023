...
public class PPSwerveControllerCommandEx extends CommandBase {
  ...
  private static BiConsumer<Translation2d, Rotation2d> logError =
      PPSwerveControllerCommandEx::defaultLogError;
   ...
   */
  public PPSwerveControllerCommandEx(
  ...
  public void execute() {
    ...
    Pose2d adjustedPose = new Pose2d(currentPose.getTranslation(), new Rotation2d(currentPose.getRotation().getRadians()));

    ChassisSpeeds targetChassisSpeeds = this.controller.calculate(adjustedPose, desiredState);

    ChassisSpeeds adjustedTargetChassisSpeeds = new ChassisSpeeds(targetChassisSpeeds.vxMetersPerSecond, targetChassisSpeeds.vyMetersPerSecond, targetChassisSpeeds.omegaRadiansPerSecond);

    if (this.useKinematics) {
      SwerveModuleState[] targetModuleStates = this.kinematics.toSwerveModuleStates(adjustedTargetChassisSpeeds);

      this.outputModuleStates.accept(targetModuleStates);
    } else {
      this.outputChassisSpeeds.accept(adjustedTargetChassisSpeeds);
    }
  ...
  public static void setLoggingCallbacks(
    ...
    PPSwerveControllerCommandEx.logActiveTrajectory = logActiveTrajectory;
    PPSwerveControllerCommandEx.logTargetPose = logTargetPose;
    PPSwerveControllerCommandEx.logSetpoint = logSetpoint;
    PPSwerveControllerCommandEx.logError = logError;
  }
}
